...
`rtl/bcmc_observer.v` is written only once the model is green, to satisfy it. It
now is, and so are `sim/bcmc_observer_hw_test.cpp` and `sim/tb_observer.v`: the
engine is implemented, lint-clean under both simulators, and the cycle-by-cycle
vectors it replays are regenerated by `validation/gen_observer_hw_vectors.py`.
Nothing in sections 3 to 6 changed in the doing — which is the point, and the
only claim here that is worth anything.
